$(function() {
    ...
    $('#sample9').click( function() {
        textarea.html('Разработка метода оценки научных статей с помощью машинного обучения\n' +
            ...
            '\n' +
            'Для решения обозначенной проблемы требуется рассмотреть и проанализировать существующие подходы к оценке научных статей. Целью данной работы является разработка классификатора для анализа научных статей на основе выбранных критериев оценки. Объектом данного исследования является анализ научных статей с точки зрения удовлетворения этим критериям. Предмет исследования - выбор критериев оценки научных статей и дальнейшая классификация научных статей.\n' +
            '\n' +
            'Обзор предметной области\n' +
            ...
            '\n' +
            'Методы, с помощью которых осуществляется анализ научных статей, могут учитывать стилистику текста (принадлежность к научному стилю), структуру, наукометрические показатели статьи, автора и издания, в котором статья опубликована, количество просмотров, добавлений в избранное и число скачиваний в электронных библиотеках. Анализ научных статей с точки зрения стиля текста, структуры, наукометрических показателей, количеству просмотров, добавлений в избранное и скачиваний в электронной библиотеке (Киберленинка), а также с учетом авторитетности издания, в котором статья была опубликована.\n' +
            '\n' +
            'Существуют следующие подходы для анализа и оценки научных статей:\n' +
            '\n' +
            'Проверка текста на соответствие научному стилю В источнике [1] предлагается анализ текста с помощью некоторых его параметров - тошноты, водности и "естественности" (значение отклонения текста статьи от идеальной кривой по Ципфу).\n' +
            '\n' +
            'Анализ наукометрических показателей Наукометрические показатели (индекс Хирша, индекс цитирования научных статей и др.) статьи, автора или издания могут помочь предсказать соответствующие показатели у анализируемой статьи. Такой метод используется в Scopus и SciVal.\n' +
            '\n' +
            'Анализ структуры научной статьи "Качество" научной статьи, с точки зрения соответствия некоторым критериям, не зависит лишь от смыслового содержания текста. Научные статьи должны соблюдать формальные требования по структуре документа и его оформлению.\n' +
            '\n' +
            'Анализ межстатейных метрик качества В источнике [2] предлагается анализ научных статей, учитывающий взаимосвязи между статьями. Данный метод позволяет получить количественную оценку свойств научных публикаций на основе ссылок на другие статьи.\n' +
            '\n' +
            'Анализ статей с точки зрения оригинальности текста и избыточного цитирования Научные статьи должны быть уникальны и не должны содержать избыточного цитирования. Использования этого метода анализа основано на поиске дословных заимствований текста из других опубликованных статей и позволяет редакторам не публиковать неуникальные тексты.\n' +
            '\n' +
            ...
            '\n' +
            'Возможность проверки статьи до ее публикации Для анализа научных статей молодыми учеными необходимо иметь возможность оценить свойства текста статьи до публикации. В рамках этой работы возможность проверки статьи до публикации необходима.\n' +
            '\n' +
            'Наличие реализации Существование приложения или репозитория для проверки работоспособности метода.\n' +
            '\n' +
            'Полнота оценивания Используемый метод должен комплексно оценивать научную статью по многим критериям. Исходя из изученных подходов, можно выделить не менее 30 различных критериев оценки статьи. В таблице сравнения значение полноты может быть обозначено как "низ.", в случае, если изучаемый метод оценивает менее 5 критериев, и "выс.", если метод оценивает более 15 критериев.\n' +
            '\n' +
            ...
            '\n' +
            'Заключение\n' +
            'Был создан обзор различных подходов, позволяющих анализировать и оценивать научные статьи с различных точек зрения. Полученный обзор позволяет оценить существующие методы в рамках выбранных критериев. Развитие этого исследования может происходить следующим образом:\n' +
            '\n' +
            'Выбор такого набора критериев оценки статьи, которые характеризуют ее наиболее полно\n' +
            'Создание программы, позволяющей собрать набор статей и получить выбранные на первом шаге критерии оценки для конкретных статей.\n' +
            'Создание и обучение классификатора, позволяющего оценивать статьи с точки зрения выбранных критериев; исследование полученного классификатора с точки зрения точности предсказания.\n' +
            'Список литературы\n' +
            'Блеес Э.И., Заславский М.М. Критерии соответствия текста научному стилю. – 2019.\n' +
